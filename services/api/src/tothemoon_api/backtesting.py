...
def _blocked_metrics(request: BacktestRequest, checklist_score: int, reason: str) -> BacktestMetrics:
...
def run_walk_forward(request: WalkForwardRequest, max_position_size_pct: float) -> WalkForwardResult:
    candles = _build_candles_for_request(request)
    signals = build_signals(request.strategy_id, candles)
