...
class CircleDeveloperClient:
    ...
    def create_wallet(self, wallet_set_id: str, idempotency_key: str | None = None) -> dict:
        ...
        response = httpx.post(f"{self.base_url}/developer/wallets", headers=self.headers, json=payload)
    ...
    def fund_with_testnet_usdc(self, address: str) -> dict:
        ...
        response = httpx.post("https://api.circle.com/v1/faucet/drips", headers=self.headers, json=payload)
        response.raise_for_status()
        return response.json()

    def execute_smoke_transfer(self, wallet_id: str, destination_address: str, amount: str, token_id: str, idempotency_key: str | None = None) -> dict:
        ...
        response = httpx.post(f"{self.base_url}/developer/transactions/transfer", headers=self.headers, json=payload)
        response.raise_for_status()
        return response.json()
